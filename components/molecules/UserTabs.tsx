...
function Tabs({ items, defaultKey, baseUrl = "/user" }: UserTabsProps) {
  ...
  return (
    <div>
      <div className="sticky top-0 z-10 bg-white/70 backdrop-blur supports-[backdrop-filter]:bg-white/60 rounded-xl border border-gray-200 p-1 mb-4 sm:mb-6 overflow-x-auto">
        <div className="flex flex-nowrap sm:flex-wrap gap-1 min-w-max sm:min-w-0">
          {items.map(item => (
            <button
              key={item.key}
              onClick={() => handleTabChange(item.key)}
              className={`px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm md:text-base transition-colors whitespace-nowrap flex-shrink-0 ${
                active === item.key
            ...
            >
              <span className="inline-flex items-center gap-1.5 sm:gap-2">
                <span className="w-3 h-3 sm:w-4 sm:h-4 flex items-center justify-center">{item.icon}</span>

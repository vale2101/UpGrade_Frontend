      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 sm:p-5 md:p-6 mb-4 sm:mb-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
          <div className="flex items-center gap-3 sm:gap-4">
            <div className="h-10 w-10 sm:h-12 sm:w-12 md:h-14 md:w-14 rounded-full bg-black text-white flex items-center justify-center font-semibold text-sm sm:text-base">
              {initials}
            </div>
            <div>
              <h1 className="text-lg sm:text-xl md:text-2xl font-bold">Hola, {user?.name}</h1>
              <p className="text-gray-600 text-xs sm:text-sm">Gestiona tus pedidos y tu información personal</p>
            </div>
          </div>
          <button onClick={handleLogout} className="w-full sm:w-auto inline-flex items-center justify-center gap-2 text-red-600 hover:bg-red-50 border border-red-200 px-3 py-2 rounded-md text-sm sm:text-base transition-colors">
            <LogOut size={16} /> <span className="sm:hidden md:inline">Cerrar sesión</span><span className="hidden sm:inline md:hidden">Cerrar</span>
          </button>
        </div>
      </div>
